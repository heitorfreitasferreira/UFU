...
public class Savanica extends Vegetacao {
  protected static double PROB_VEGETACAO = 1;
  public static final int codigo = 5;

  public Savanica(Celula celula) {
    super(celula);
  }
          ...
          if ((i != 0 && j != 0) &&
              ((vizinhanca[i][j].getEstado() instanceof QueimaEstavel
                  ...
                  Math.random() < vizinhanca[i][j].getReticulado().getVento().get(i, j) * QueimaEstavel.PROB_QUEIMA
                      * Reticulado.getCoef_umidade() * Savanica.PROB_VEGETACAO)
                  ||
                  (vizinhanca[i][j].getEstado() instanceof QueimaLenta
                      &&
