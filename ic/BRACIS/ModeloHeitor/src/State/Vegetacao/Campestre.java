...
public class Campestre extends Vegetacao {
  protected static double PROB_VEGETACAO = 0.6;
  public static final int codigo = 4;

  public Campestre(Celula celula) {
    super(celula);
  }
          ...
          if ((i != 0 && j != 0) &&
              ((vizinhanca[i][j].getEstado() instanceof QueimaEstavel
                  ...
                  Math.random() < vizinhanca[i][j].getReticulado().getVento().get(i, j) * QueimaEstavel.PROB_QUEIMA
                      * Reticulado.getCoef_umidade() * Campestre.PROB_VEGETACAO)
                  ||
                  (vizinhanca[i][j].getEstado() instanceof QueimaLenta
                      &&
