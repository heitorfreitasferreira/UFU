...
#define TAMANHO_RETICULADO 1025
...
Celula heitorzera1(Celula n,Celula s,Celula o,Celula l,Celula ne,Celula no,Celula se,Celula so,Celula centro){
    ...
    if(centro.estado == solo_exposto) centro.tempo_desde_queima++;
    if(centro.estado >= inicio_fogo && centro.estado <= queima_lenta) centro.tempo_queimando++;
    #pragma endregion avançandoInfoCelula
    #pragma region atualizandoEstado
    if(centro.tempo_queimando > 2 )     centro.estado = arvore_queimando;
    if(centro.tempo_queimando > 6)      centro.estado = queima_lenta;
...
int deterministico(int n,int s,int o,int l,int ne,int no,int se,int so,int centro){
	...
	if(centro == arvore_viva && total>0) return arvore_queimando;
	if(centro == solo_exposto && p<=0.01) return arvore_viva;
	if(centro == arvore_viva && f<=0.0005) return arvore_queimando;//0.0005
	return centro;
}
        ...
        do{
            ...
            for(size_t i = 0;i<TAMANHO_RETICULADO; i++){
                for(size_t j = 0;j<TAMANHO_RETICULADO; j++){
                    ...
                    if(i == 0)  cima = TAMANHO_RETICULADO;
                    else cima = i-1;
                    baixo = i +1;
                    if(j==0) esquerda = TAMANHO_RETICULADO;
                    else esquerda = j-1;
                    direita = j +1;
