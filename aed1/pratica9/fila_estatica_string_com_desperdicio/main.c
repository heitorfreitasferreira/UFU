...
void print_fila(Fila q, String *array)
{
  // array = (String *)malloc(sizeof(String) * max);
  int i = 0;
  while (!fila_vazia(q))
  {
    shift(q, array[i].string);
...
int main(int argc, char const *argv[])
{
  ...
  while (continuar)
  {
    ...
    case 2:
      ...
      // scanf("%d", &tmp);
      tmp = 0;
      printf("Insira a numero que deseja inserir na fila %d: ", tmp);
      setbuf(stdin, NULL);
      scanf("%[^\n]", numero);
    ...
    case 3:
      ...
      // setbuf(stdin, NULL);
      // scanf("%d", &numero);
      if (shift(stack[tmp], numero))
      {
        printf("Sucesso, %s removido\n", numero);
        // sleep(0.3);
        // system("clear"); //MUDAR PRA CLS PRA SISTEMAS INFERIORES
      }
      else
